...
"""
MoveJ Motion Program Test
========================

This test demonstrates how to execute a joint-space (MoveJ) motion program with a robot arm using the Motorcortex Robot Control API. It covers engaging the robot, building a joint-space motion program, handling start position logic, and monitoring execution.
...
4. Defines joint-space waypoints for the MoveJ motion.
5. Builds and sends a joint-space motion program (MoveJ).
6. Handles cases where the robot is not at the start position.
7. Monitors and prints the robot state during execution.
8. Repeats the motion program to demonstrate multiple executions.
9. Closes the connection after completion.

Usage:
------
Run this file to see an example of executing a joint-space motion with a robot arm. Adjust waypoints and connection parameters as needed for your setup.
"""
...
def main():
    ...
    # Step 7: Define joint-space waypoints for the MoveJ motion
    jpos_1 = Waypoint(
    ...
    # Step 8: Build joint-space motion program (MoveJ)
    ...
    # Step 11: Repeat the motion program to demonstrate multiple executions
    motion_pr_2.send("test3").get()
    while robot.getState() is InterpreterStates.PROGRAM_IS_DONE.value:
        time.sleep(0.1)
        print('Waiting for the program to start, robot state: {}'.format(robot.getState()))
