...
The `service_config.json` file is a JSON configuration file that controls how the Debian package is built. You can override any option from the default by specifying it in your config. Any missing option will fall back to the default (see `default_package_config.json`).
