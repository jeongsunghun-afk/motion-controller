"""mujoco_node.py — gait_sim_mujoco

unitree_mujoco 의 simulator 역할. motorcortex_bridge 의 joint_state_bridge.py
+ motion_controller.py 의 RL_POLICY control flow 와 동일 인터페이스/타이밍:

    sub: /low_cmd          (sensor_msgs/JointState)
         · position = q_target (20-dim)
         · velocity = qdot_target (20-dim, optional)
         · effort   = tau_ff (20-dim, optional)
    pub: /joint_states     (50 Hz, RViz 호환)
    pub: /low_state        (50 Hz, Unitree LowState 호환 — q_meas / qdot_meas)
    pub: /imu              (200 Hz, base orientation/gyro/accel — r4.1 EKF 용)
    sub: /rl_gains         (옵션: Kp/Kd 동적 갱신)

내부 control flow:
   _control_loop @ 200 Hz:
     · /low_cmd 의 q_target / qdot_target / tau_ff Hermite C1 보간 (50 Hz → 200 Hz)
     · tau_motor = Kp(q_interp − q_meas) + Kd(qdot_interp − qdot_meas) + tau_ff
     · data.ctrl[:] = clip(tau_motor, ±τ_max)
     · mujoco.mj_step()
     · (옵션) viewer.sync()

v14.5.9-a 초기 골격: standing pose 유지 가능 목표.
"""
import math
import os
import threading
...
SIM_DT_DEFAULT    = 0.005    # 200 Hz control loop (motorcortex 와 동일)
CMD_PERIOD        = 0.02     # /low_cmd 수신 주기 가정 (50 Hz)
PUBLISH_HZ_LOWS   = 50       # /joint_states + /low_state
PUBLISH_HZ_IMU    = 200
N_DOF             = 20       # 4 legs × 5 joints (mujoco mjcf joint order: FR/FL/HR/HL × j1..j5)
KP_DEFAULT        = 100.0
KD_DEFAULT        = 5.0
TAU_MAX           = 200.0    # Nm (mjcf actuator ctrlrange 와 일치)

# Q_HOME (gait_sim model.py 와 일치, leg-major)
...
BASE_Z_HOME = 0.465  # standing 자세에서 base 높이 (실측 기반)


# ── Hermite C1 보간 (motorcortex_bridge motion_controller.py 패턴 mirror) ────
def _hermite_pos(s, T, q0, q1, m0, m1):
    """3차 Hermite 위치 보간 (s ∈ [0,1], T = 구간 시간)."""
    h00 = 2*s**3 - 3*s**2 + 1
    h10 = s**3 - 2*s**2 + s
    h01 = -2*s**3 + 3*s**2
    h11 = s**3 - s**2
...
class MujocoSimNode(Node):
    """gait_sim_mujoco simulator node — unitree_mujoco style.

    토픽 인터페이스는 motorcortex_bridge 와 동일하여 controller 가 sim/real
    구분 없이 사용 가능. launch file 한 줄 swap 으로 sim ↔ real 전환.
    """
    ...
    def __init__(self):
        ...
        # joint names (MuJoCo joint order)
        self._joint_names = []
        for jid in range(1, self._model.njnt):  # skip freejoint 0
            n = mujoco.mj_id2name(self._model, mujoco.mjtObj.mjOBJ_JOINT, jid)
            if n is not None:
                self._joint_names.append(n)
        self.get_logger().info(
            f"MJCF loaded: {self._model.nu} actuators, "
            f"joints: {self._joint_names[:4]}..."
        )

        # ── 초기 자세: Q_HOME + base z = BASE_Z_HOME (floor 와 contact) ──
        self._data.qpos[0:3] = [0.0, 0.0, BASE_Z_HOME]
        self._data.qpos[3:7] = [1.0, 0.0, 0.0, 0.0]   # quat (w,x,y,z) identity
        self._data.qpos[7:]  = Q_HOME_RAD              # 다리 home pose
        mujoco.mj_forward(self._model, self._data)

        # ── Hermite 보간 state (Q_HOME 으로 초기화) ─────────────────────
        ...
        # ── 타이머 ───────────────────────────────────────────────────────
        # control_loop @ sim_dt (5ms = 200 Hz)
        self._ctrl_timer = self.create_timer(self._sim_dt, self._control_loop)
        # state publish @ 50 Hz
        ...
        self.get_logger().info(
            f"MujocoSimNode ready | sim_dt={self._sim_dt}s | "
            f"viewer={self._use_viewer} | kp_default={kp_init} kd_default={kd_init}"
        )

    # ── /low_cmd subscribe ───────────────────────────────────────────────
    def _on_low_cmd(self, msg: JointState):
        """motion_controller.py:set_command() pattern.
        q_target = msg.position (필수, N_DOF dim)
        qdot_target = msg.velocity (옵션)
        tau_ff = msg.effort (옵션)
        """
        ...
        now = time.monotonic()
        with self._lock:
            # 현재 Hermite 진행률에서 새 보간 segment 시작 (C1 연속)
    ...
    def _control_loop(self):
        """매 sim_dt 마다 Hermite 보간된 q_target + PD + tau_ff → MuJoCo step.
        motion_controller.py:_control_loop 의 RL_POLICY 상태와 같은 패턴.
        """
        now = time.monotonic()
        with self._lock:
            elapsed = now - self._interp_time
    ...
    def _publish_states(self):
        ...
        self._pub_low_state.publish(js)   # Unitree LowState 호환 (동일 형태)

        # IMU (200Hz 시간이 50Hz 와 다르지만 일단 50Hz 에 같이 publish)
        imu = Imu()
        imu.header.stamp = stamp
        imu.header.frame_id = 'base_link'
        # orientation (base quaternion, w,x,y,z → ROS x,y,z,w)
        ...
        # angular velocity (world frame; r4.1 EKF 에서 body frame 변환)
        ...
        # linear acceleration (qacc 의 linear part — gravity 포함)
